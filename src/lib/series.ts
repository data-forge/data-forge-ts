...
export interface ISeries<IndexT = number, ValueT = any> extends Iterable<ValueT> {
   ...
   toArray (): ValueT[];
     ...
     * The transfomration function is called for each value in the input series and produces a new value that is output to the generated series.
     * 
     ...
     *
     * @param transformFn A user-defined transformation function that transforms each value to a new value included in the generated series.
     * 
     * @return Returns the series generated by calling the transformation function over each value in the input series.
     * 
     * @example
     * <pre>
     * 
     ...
     * 
     * @param transformationFn A user-defined function that transforms each value into an array that is flattened into the generated series.
     * 
     * @return Returns the series generated by calling the transformation function over each value in the input series.
     * 
     * @example
     ...
     * }
     * 
     * const transformed = series.selectMany(value => produceOutputValues(value));
     * console.log(transformed);
     * </pre>
     */
    selectMany<ToT> (transformationFn: SelectorWithIndexFn<ValueT, Iterable<ToT>>): ISeries<IndexT, ToT>;
     ...
     * }
     * 
     * const transformed = series.flatMap(value => produceOutputValues(value));
     * console.log(transformed);
     * </pre>
     */
    flatMap<ToT> (transformationFn: SelectorWithIndexFn<ValueT, Iterable<ToT>>): ISeries<IndexT, ToT>;
...
export class Series<IndexT = number, ValueT = any> implements ISeries<IndexT, ValueT> {
   ...
   toArray (): any[] {
     ...
     */
    selectMany<ToT> (transformationFn: SelectorWithIndexFn<ValueT, Iterable<ToT>>): ISeries<IndexT, ToT> {
        if (!isFunction(transformationFn)) throw new Error("Expected 'transformationFn' parameter to 'Series.selectMany' to be a function.");

        return this.flatMap<ToT>(transformationFn);
    }
     ...
     */
    flatMap<ToT> (transformationFn: SelectorWithIndexFn<ValueT, Iterable<ToT>>): ISeries<IndexT, ToT> {
        if (!isFunction(transformationFn)) throw new Error("Expected 'transformationFn' parameter to 'Series.flatMap' to be a function.");
        ...
        return new Series(() => ({
            pairs: new SelectManyIterable(
                ...
                (pair: [IndexT, ValueT], index: number): Iterable<[IndexT, ToT]> => {
                    ...
                    for (const transformed of transformationFn(pair[1], index)) {
                        outputPairs.push([
                            pair[0],
                            transformed
                        ]);
